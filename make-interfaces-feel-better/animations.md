---
name: animations
description: Specific micro-interaction techniques — interruptible animations, stagger, scale-on-press, icon animation transitions, pressed states. The element-level polish layer. Use for hover/press/focus feedback, icon state changes, list item stagger, and any single-element motion tuning.
scope: sub-agent
triggers: interruptible, stagger, scale on press, scale-on-press, icon animation, press feedback, micro-interaction, pressed state, active state, :active, bounce, wiggle, transform scale, haptic, tap feedback, ripple, morph icon
---
