---
name: typography
description: Type rendering details — font-smoothing (antialiased), tabular-nums for changing numerals, text-wrap balance/pretty, text-balance for headings, optical letter-spacing. The fine-tuning layer above font choice itself. Use for any screen with numbers that change (prices, timers, stats), headings that need to balance, or UI where typographic precision matters.
scope: sub-agent
triggers: tabular-nums, font-smoothing, antialiased, text-wrap, text-wrap balance, text-balance, text-pretty, optical alignment, kerning, letter-spacing, line-height, leading, variable font, font-feature-settings
---
