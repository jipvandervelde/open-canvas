---
name: performance
description: Rendering and compositing performance — transition specificity (never `transition: all`), will-change hints, transform-only animation, paint/layer considerations. Use when tuning element-level animation perf, avoiding jank, or when a screen has many simultaneously-animating elements.
scope: sub-agent
triggers: transition all, transition specificity, will-change, transform only, transform-only, paint, layer, gpu, composite, jank, fps, animation perf, translateZ, backdrop-filter, contain
---
