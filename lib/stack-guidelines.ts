...
Component-level styling lives in a projected Sandpack file at \`/component-tokens.js\`. Every seeded component (Button, Card, TextField, Switch, SegmentedControl, TabBar) reads its surface/ink/radius/padding from there. Agent-generated screens should do the same when they want to match the system:
...
Available entries: \`button-primary\`, \`button-primary-pressed\`, \`button-primary-disabled\`, \`button-secondary\`, \`button-secondary-pressed\`, \`button-ghost\`, \`button-ghost-pressed\`, \`card\`, \`card-interactive-pressed\`, \`text-field\`, \`text-field-label\`, \`text-field-focus-ring\`, \`text-field-error-ring\`, \`text-field-error-message\`, \`text-field-helper-message\`, \`switch-track-off\`, \`switch-track-on\`, \`switch-thumb\`, \`segmented-control\`, \`segmented-control-pill\`, \`segmented-control-label-active\`, \`segmented-control-label-inactive\`, \`tab-bar\`, \`tab-item-active\`, \`tab-item-inactive\`, \`focus-ring\`. The \`components:\` section of the DESIGN.md YAML front matter (prepended to design.md) is the authoritative list — check it for the current contract.
...
- \`./components/{Name}.js\` — shared UI primitives (Button, Card, ListRow, TabBar, EmptyState). PascalCase name. Import as \`import Name from './components/Name';\`. Idempotent by name — calling createComponent again with the same name updates in place.
...
- **Search BEFORE writing \`<Icon name="…">\`.** Call \`searchIcons({ query: "chart statistics" })\` to get exact names; guessing will render null + print a console warning at runtime. Names are specific: "IconBellRing" not "IconBellAlert", "IconChart1" not "IconBarChart".
