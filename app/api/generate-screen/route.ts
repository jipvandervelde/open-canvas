...
- Import shared components (\`import Name from './components/Name';\`), services (\`import { useSession } from './services/session';\`, \`import { Link, useParams } from './services/router';\`, \`import { useToast } from './services/toast';\`), motion presets (\`import { Motion, MotionList } from './motion';\`), and data entities (\`import { recipes, findRecipe } from './data/recipes';\`) from the paths listed in the shared context. Do NOT invent imports — only use what's listed. The component registry includes aliases/tags/useWhen/avoidWhen; if a registry component covers the pattern, import it and pass props instead of recreating the markup inline. For bottom tabs / bottom nav / primary mobile tabs, use \`BottomTabBar\` exactly, not TabBar and not inline nav markup.
...
- For list screens, import the data entity and render rows from it. If the entity exports \`{entityName}Ready\` and it is false, render a loading/empty state from that shared entity — do NOT create fallback rows in screen code. For detail screens, \`const { id } = useParams();\` then \`find{Singular}(id)\`. For list→detail links use querystring params: \`<Link to={\\\`/name-detail?id=\${item.id}\\\`}>\`.
