...
/**
 * Open-canvas integration: restrict agentation's element picker to the canvas
 * artboard area so the user can annotate screens but not the surrounding app
 * chrome (chat panel, preview panel, toolbar, dialkit popover, etc.). Treats
 * the nearest ancestor with `[data-agentation-scope]` as the allowed root, and
 * anything marked `[data-agentation-ignore]` as an explicit skip.
 */
function isOutsideAgentationScope(el: Element | null): boolean {
  if (!el) return true;
  // Explicit opt-out wins even if technically inside the scope.
  if (closestCrossingShadow(el, "[data-agentation-ignore]")) return true;
  // If a scope container exists in the document, only allow its descendants.
  if (typeof document !== "undefined" && document.querySelector("[data-agentation-scope]")) {
    return !closestCrossingShadow(el, "[data-agentation-scope]");
  }
  return false;
}
...
export type PageFeedbackToolbarCSSProps = {
  ...
  className?: string;
  /** External hover feed — when set, agentation uses this instead of its own
   *  `deepElementFromPoint` pick. Used to bridge hovers from inside iframes
   *  (Sandpack, etc.) whose DOM is unreachable from the parent document. */
  externalHover?: {
    clientX: number;
    clientY: number;
    elementName: string;
  ...
  /** External click feed — when this changes to a non-null value, agentation
   *  opens a new pending annotation using the payload. Token field forces
   *  re-trigger on repeat clicks with the same element. */
  externalClick?: {
    token: number;
    clientX: number;
    clientY: number;
  ...
  /** Fires when agentation's active state changes — used by the bridge to
   *  arm/disarm the in-iframe listeners via postMessage. */
  onActiveChange?: (isActive: boolean) => void;
  /** Fires when the user clicks the popup's "Fix" button for a specific
   *  annotation. The bridge packages the annotation into a chat prompt. */
  onAnnotationFix?: (annotation: Annotation) => void;
  /** Live-position override — when set, agentation calls this for every
   *  rendered annotation before positioning its pin. Used by the iframe
   *  bridge to keep pins locked to iframe-local coordinates when the
   *  enclosing canvas is panned/zoomed. Return `null` to keep the
   *  annotation's stored coordinates. */
  ...
  /** Tick value the bridge bumps on every canvas change. Used purely to
   *  trigger re-renders; a stable number means pins stay put. */
  positionTick?: number;
};
...
export function PageFeedbackToolbarCSS({
  ...
  // Notify parent when active state changes — lets the iframe bridge arm
  // the in-iframe selection agent (switch to "agentation" mode) on activation
  // and disarm it on deactivation.
  useEffect(() => {
...
const [settings, setSettings] = useState<ToolbarSettings>(() => {
  ...
  // External hover feed — when the iframe bridge reports a hover from inside
  // a Sandpack-like nested document, route it straight into the hover state.
  // Skip when a pending annotation or drawing is active so we don't clobber
  // the in-progress UX.
  useEffect(() => {
    if (!isActive || pendingAnnotation || isDrawMode || isDesignMode) return;
    if (!externalHover) {
      // Clear only if the previous frame was an external one; don't step on
      // a regular parent-DOM hover.
      return;
    }
    setHoverInfo({
      element: externalHover.element,
      elementName: externalHover.elementName,
      elementPath: externalHover.elementPath,
      rect: new DOMRect(
        externalHover.rect.x,
        externalHover.rect.y,
        externalHover.rect.width,
        externalHover.rect.height,
      ),
      reactComponents: externalHover.reactComponents ?? null,
    });
    setHoverPosition({ x: externalHover.clientX, y: externalHover.clientY });
  }, [externalHover, isActive, pendingAnnotation, isDrawMode, isDesignMode]);

  // External click feed — bridge reports a click inside an iframe; open a
  // new pending annotation pinned to that element.
  useEffect(() => {
    if (!externalClick || !isActive) return;
    if (pendingAnnotation || editingAnnotation) return;
    const x = (externalClick.clientX / window.innerWidth) * 100;
    const y = externalClick.clientY + window.scrollY;
    setPendingAnnotation({
      x,
      y,
      clientY: externalClick.clientY,
      element: externalClick.element,
      elementPath: externalClick.elementPath,
      selectedText: externalClick.selectedText,
      boundingBox: {
        x: externalClick.rect.x,
        y: externalClick.rect.y + window.scrollY,
        width: externalClick.rect.width,
        height: externalClick.rect.height,
      },
      isFixed: false,
      reactComponents: externalClick.reactComponents ?? undefined,
    });
    setHoverInfo(null);
    // Re-runs on every new token even if the other fields match — lets the
    // user drop two pins on the same element in a row.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [externalClick?.token, isActive]);
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (
        ...
        isOutsideAgentationScope(target)
      ) {
        setHoverInfo(null);
        return;
      }
      ...
      if (
        ...
        isOutsideAgentationScope(elementUnder)
      ) {
        setHoverInfo(null);
        return;
      }
  ...
  useEffect(() => {
    ...
    const handleClick = (e: MouseEvent) => {
      ...
      if (isOutsideAgentationScope(target)) return;
  ...
  // Add annotation. Pass `keepOpen=true` when the popup should stay alive
  // after the first save (autosave mode — see AnnotationPopupCSS). In that
  // mode we promote the pending annotation into an editable one in place
  // rather than animating the popup out.
  const addAnnotation = useCallback(
    (comment: string, keepOpen: boolean = false) => {
      ...
      if (keepOpen) {
        // "Save and keep editing" path — used when the popup is in autosave
        // mode and the user presses Enter. We swap pending → editing in
        // place so the popup stays alive and subsequent edits route through
        // updateAnnotation.
        setPendingAnnotation(null);
        setPendingExiting(false);
        setEditingAnnotation(newAnnotation);
      } else {
        // Legacy flow — animate the popup out.
  ...
  // Append a reply (thread message) to the currently-editing annotation.
  // Used by the popup's Reply button so subsequent entries after the first
  // comment accumulate as a thread instead of overwriting each other.
  const appendReply = useCallback(
    (content: string) => {
      ...
      const updated: Annotation = {
        ...editingAnnotation,
        thread: [...(editingAnnotation.thread ?? []), nextMsg],
      };
      setAnnotations((prev) =>
        prev.map((a) => (a.id === editingAnnotation.id ? updated : a)),
      );
      setEditingAnnotation(updated);
      onAnnotationUpdate?.(updated);
      fireWebhook("annotation.update", { annotation: updated });
    },
    [editingAnnotation, onAnnotationUpdate, fireWebhook],
  );
  ...
  const copyOutput = useCallback(async () => {
    ...
    if (copyToClipboard) {
      // Chrome rejects writeText with NotAllowedError when the document
      // doesn't have focus. In Open Canvas the Sandpack iframe frequently
      // holds focus, so we re-focus the parent window first — and catch
      // silently if the browser still refuses.
  ...
  // Filter annotations for rendering (exclude exiting ones from normal
  // flow). Then apply the external position override — the iframe bridge
  // uses it to keep pins anchored to elements inside Sandpack iframes as
  // the canvas pans. `positionTick` is read here so the JSX re-evaluates
  // override results whenever the bridge bumps the counter.
  void positionTick;
  const visibleAnnotations = annotations
    ...
    .map((a) => {
      if (!getAnnotationPosition) return a;
      const p = getAnnotationPosition(a);
      if (!p) return a;
      return {
        ...a,
        x: p.x,
        y: p.y,
        boundingBox: p.boundingBox ?? a.boundingBox,
                ...
                return (
                  <>
                    ...
                    <AnnotationPopupCSS
                      ...
                      onSubmit={
                        onAnnotationFix
                          ? (text) => addAnnotation(text, true)
                          : addAnnotation
                      }
                      onAutoSave={undefined}
                      // onReply is a no-op pre-commit — `handleReply` routes
                      // through onSubmit until the annotation is promoted to
                      // editing — but we wire it so the popup knows this is
                      // thread mode (Reply label, threadMessages shown).
                      onReply={onAnnotationFix ? () => {} : undefined}
                      onFix={onAnnotationFix ? (text) => {
                        // addAnnotation runs first (via onSubmit inside popup's
                        // handleFix); the popup closes as part of its anim-out.
                        // Construct a minimal annotation here for the caller
                        // since the real Annotation object is created inside
                        // addAnnotation after our callback returns.
                        onAnnotationFix({
                          id: "pending",
                          x: pendingAnnotation.x,
                          y: pendingAnnotation.y,
          ...
          {editingAnnotation && (
            <>
              ...
              <AnnotationPopupCSS
                ...
                // In thread mode the textarea composes new replies — the
                // root comment + past replies render as thread bubbles
                // above, so initialValue is intentionally empty.
                initialValue={onAnnotationFix ? "" : editingAnnotation.comment}
                submitLabel="Save"
                onSubmit={updateAnnotation}
                // onAutoSave intentionally omitted in thread mode so every
                // keystroke doesn't overwrite the committed root comment.
                onFix={onAnnotationFix ? (text) => {
                  // Include any pending reply text in the payload.
                  const merged = text
                    ? {
                        ...editingAnnotation,
                        thread: [
                          ...(editingAnnotation.thread ?? []),
                          {
                            id: `${Date.now()}-f`,
                            role: "human" as const,
                            content: text,
                    ...
                    : editingAnnotation;
                  onAnnotationFix(merged);
                } : undefined}
                onReply={onAnnotationFix ? appendReply : undefined}
                threadMessages={
                  onAnnotationFix
                    ? [
                          ...
                          ? [
                              {
                                id: "root",
                                role: "human" as const,
                                content: editingAnnotation.comment,
