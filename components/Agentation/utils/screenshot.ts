...
async function getDomCapture() {
  ...
  try {
    // Optional peer dep — typed as a dynamic string so Next.js/TS doesn't
    // resolve it at build time when the package isn't installed.
    _domCaptureModule = await import(/* webpackIgnore: true */ "modern-screenshot" as string);
