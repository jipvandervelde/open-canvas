---
name: performance
description: List and data-scale performance — virtualization, infinite scroll, lazy loading, large datasets. Use when a screen renders many rows, cards, or items, or when the list/feed/catalog will grow over time.
scope: sub-agent
triggers: list, feed, catalog, search result, virtualization, virtualize, infinite scroll, lazy load, lazy-load, dashboard, many items, timeline, inbox, grid of, feed of, data table, pagination, windowing, react-window, tanstack virtual
---
...
**Scope of this file**: list/data-scale perf. Virtualization, lazy loading, pagination. The "we have 1000 items" layer. For CSS-compositing / paint perf (transition specificity, will-change, transform-only animations), see `make-interfaces-feel-better/performance`.
